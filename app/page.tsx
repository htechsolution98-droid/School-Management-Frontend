...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const p1 = fetch("/api/landing/settings")
      .then((res) => res.json())
      .then((data) => {
    ...
    // 2. Fetch Features
    const p2 = fetch("/api/landing/features")
      .then((res) => res.json())
      .then((data) => {
    ...
    const p3 = fetch("/api/landing/modules")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
    ...
    const p4 = fetch("/api/landing/testimonials")
      .then((res) => res.json())
      .then((data) => {
    ...
    const minTimer = new Promise((resolve) => setTimeout(resolve, 1800));

    Promise.allSettled([p1, p2, p3, p4, minTimer]).then(() => {
      ...
      <AnimatePresence mode="wait">
        {isPageLoading && (
          <motion.div
            ...
            className="fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-gradient-to-br from-[#1D496C] via-[#15354F] to-[#0F172A]"
          >
            {/* Spinning/glowing loader circle */}
            <div className="relative flex items-center justify-center">
              {/* Outer rotating gradient ring */}
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ repeat: Infinity, duration: 2, ease: "linear" }}
                className="h-28 w-28 rounded-full border-4 border-t-[#FFA600] border-r-transparent border-b-[#429CE4] border-l-transparent shadow-[0_0_20px_rgba(255,166,0,0.2)]"
              />

              {/* Inner scaling and rotating logo container ("logo are round up") */}
              <motion.div
                ...
                className="absolute h-20 w-20 rounded-full bg-white p-2 flex items-center justify-center shadow-2xl border border-white/10"
              >
                <img
                  src="/logo.png"
                  alt="VidyaSanchalan Logo"
                  className="h-14 w-auto object-contain"
                />
              </motion.div>
            </div>

            {/* Glowing text under loader */}
            <motion.div
            ...
            >
              <h1 className="text-2xl font-black mapping-tight text-white">
                Vidya<span className="text-[#FFA600]">Sanchalan</span>
              </h1>
              <p className="text-[10px] text-white/60 font-bold uppercase mapping-widest animate-pulse">
      ...
      <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#F8FAFC] via-white to-[#429CE4]/5">
          ...
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex h-16 items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xl font-black mapping-tight"><span className="text-white">Vidya</span><span className="text-[#FFA600]">Sanchalan</span></span>
                  <p className="text-[10px] text-white/70 font-bold uppercase mapping-wider mt-0.5">School Management</p>
                </div>
              </div>
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {/* Static background blue gradient blobs */}
            <div className="absolute top-0 left-0 h-[500px] w-[500px] rounded-full bg-gradient-to-r from-[#429CE4]/15 via-[#285E89]/15 to-[#1D496C]/10 blur-[90px]" />
            <div className="absolute bottom-0 right-0 h-[500px] w-[500px] rounded-full bg-gradient-to-r from-[#285E89]/10 via-[#429CE4]/15 to-[#FFA600]/10 blur-[90px]" />
          </div>
            ...
            <div className="grid lg:grid-cols-12 gap-12 items-center">
              ...
              <div className="lg:col-span-7 space-y-8 text-left">
                <div className="space-y-4">
                  <Badge
                    variant="outline"
                    className="rounded-full px-4 py-1.5 border-[#6A7626]/30 bg-[#6A7626]/10 text-[#6A7626] shadow-sm font-bold mapping-wider uppercase text-xs"
                  >
                    {heroBadge}
                  </Badge>

                  <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black mapping-tight leading-[1.15]">
                    <span className="bg-gradient-to-r from-[#285E89] to-[#FFA600] bg-clip-text text-transparent block font-black mb-2 text-5xl sm:text-6xl lg:text-7.5xl mapping-tighter">
                      {heroTitle}
                    </span>
              ...
              <div className="lg:col-span-5 relative flex justify-center">
                ...
                <div className="absolute -inset-4 bg-gradient-to-tr from-[#429CE4]/30 to-[#6A7626]/20 rounded-[2.5rem] blur-2xl opacity-70 -z-10"></div>

                {/* Static 3D image frame */}
                <div className="relative overflow-hidden rounded-[2.5rem] border-4 border-white bg-slate-100 shadow-2xl shadow-slate-900/10 hover:shadow-slate-900/20 transition-shadow duration-500 max-w-[480px] w-full aspect-[4/3] flex items-center justify-center group">
                  <AnimatePresence mode="wait">
                    <motion.img
                      key={currentHeroIdx}
                    ...
                    <div className="absolute bottom-[5.5rem] left-1/2 -translate-x-1/2 flex gap-1.5 z-20 bg-slate-950/60 px-3 py-1 rounded-full border border-white/10 backdrop-blur-sm">
                      {heroImages.map((_, idx) => (
                        <button
                          ...
                          className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${idx === currentHeroIdx
                  ...
                  <div className="absolute bottom-6 left-6 right-6 backdrop-blur-md bg-white/80 border border-white/20 p-4 rounded-2xl shadow-lg flex items-center justify-between">
                    <div>
                      <p className="text-[10px] uppercase mapping-wider text-[#6A7626] font-bold">Admission Portal</p>
                ...
                {/* Additional static card to create a 3D overlay aesthetic */}
                <div className="absolute -top-6 -right-4 backdrop-blur-md bg-white/90 border border-slate-100 px-4 py-3 rounded-2xl shadow-lg hidden sm:flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-[#1D496C]">Geo mapping Enabled</p>
                  </div>
                </div>
              </div>
          ...
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="space-y-6">
              <div className="text-center mb-2">
                <span className="text-xs font-bold uppercase mapping-wider text-[#1D496C]/75">VidyaSanchalan Ecosystem Modules & Highlights</span>
              </div>
            ...
            <div className="grid lg:grid-cols-12 gap-12 items-center">
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-20 bg-slate-950/60 px-3 py-1 rounded-full border border-white/10 backdrop-blur-sm">
                      {aboutImages.map((_, idx) => (
                        <button
                          ...
                          className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${idx === currentAboutIdx
                              ? "bg-[#FFA600] scale-125 w-3.5"
                              : "bg-white/40 hover:bg-white"
                            }`}
                          aria-label={`Go to slide ${idx + 1}`}
              ...
              >
                <div className="space-y-3">
                  ...
                  <h2 className="text-3.5xl sm:text-4xl lg:text-5xl font-black mapping-tight text-[#1D496C] leading-tight">
                  ...
                  <div>
                    ...
                    {feature.points.length > 4 && (
                      <button
                        ...
                        className={`mt-4 text-xs font-black transition-colors inline-flex items-center gap-1.5 hover:underline ${activeFeatureIdx === index
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 items-center">
                ...
                <div className="relative w-full max-w-[340px] sm:max-w-[420px] aspect-square flex items-center justify-center group">
                  ...
                  >
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-30 bg-slate-950/60 px-3 py-1 rounded-full border border-white/10 backdrop-blur-sm">
                        {whyImagesMain.map((_, idx) => (
                          <button
                            ...
                            className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${idx === currentWhyIdx
                                ? "bg-[#FFA600] scale-125 w-3.5"
                                : "bg-white/40 hover:bg-white"
                              }`}
                            aria-label={`Go to slide ${idx + 1}`}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-500 font-extrabold uppercase mapping-wider">User Satisfaction</div>
                      </div>
                    </motion.div>
                  </motion.div>
                  ...
                  <div className="absolute bottom-[-4.5rem] left-[1rem] sm:left-[3rem] md:left-0 flex flex-wrap gap-2 z-40 max-w-[340px]">
                    {whyPills.map((pill, idx) => {
                      ...
                      return (
                        <span key={idx} className={`inline-flex items-center px-4 py-2 rounded-full text-white text-[10px] font-black mapping-wide shadow-lg hover:scale-105 transition-transform cursor-pointer border-0 ${activeBg}`}>
                          {pill}
                        </span>
                      );
                    })}
              ...
              >
                ...
                <div className="space-y-6">
                  {/* Purple pill badge */}
                  <div>
                    <span className="inline-flex items-center px-3 py-1 rounded-full bg-[#5D3FD3]/10 text-[#5D3FD3] text-[11px] font-black mapping-wide uppercase">
                      {whyBadge}
                    </span>
                  </div>

                  {/* Main Styled Heading */}
                  <h2 className="text-2xl sm:text-3xl lg:text-[2.1rem] font-extrabold mapping-tight text-slate-900 leading-[1.2]">
                ...
                >
                  {stats.map((stat, i) => (
                    ...
                    >
                      ...
                      <div className="text-3xl font-extrabold mb-2 mapping-tight group-hover:text-[#FFA600] transition-colors duration-300">
        ...
        <section className="pt-10 pb-6 bg-[#F8FAFC]">
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-8">
              ...
              <h2 className="text-3xl font-bold mapping-tight sm:text-4xl mb-4 text-[#0F172A]">
                ...
                <div className="flex justify-center gap-2.5 mt-8 relative z-20">
                  {testimonialsList.map((_, index) => (
                    <button
                      ...
                      className={`h-2.5 rounded-full transition-all duration-500 ${index === currentTestimonial
                          ? "w-8 bg-[#1D496C]"
            ...
            <Card className="border-0 bg-gradient-to-br from-[#1D496C] to-[#15354F] text-white shadow-2xl overflow-hidden rounded-[2.5rem] cursor-pointer">
              ...
              <CardContent className="p-16 text-center relative">
                <div className="max-w-2xl mx-auto space-y-6">
                  ...
                  <h2 className="text-3xl font-black mapping-tight sm:text-4xl lg:text-5xl text-white">
        ...
        <footer className="border-t border-[#6A7626]/20 bg-white">
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12 pb-6">
            <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-5">
              ...
              <div className="lg:col-span-2">
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative h-20 w-auto flex items-center justify-center rounded-xl bg-white p-2 shadow-md border border-[#1D496C]/10">
                    <img src="/logo.png" alt="VidyaSanchalan Logo" className="h-16 w-auto max-w-[160px] object-contain" />
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] text-[#475569]/80 font-bold uppercase mapping-wider mt-0.5">School Management</p>
                  </div>
                </div>
              ...
              {/* Product Column */}
              <div>
                <h3 className="font-bold text-[#0F172A] mb-4 text-sm uppercase mapping-wider">Product</h3>
                <ul className="space-y-3">
                  {[
                    ...
                    <li key={j}>
                      <Link href={link.href} className="text-sm text-[#475569] hover:text-[#285E89] transition-colors font-medium">
                        {link.label}
                      </Link>
                    </li>
                  ))}
              ...
              {/* Company Column */}
              <div>
                <h3 className="font-bold text-[#0F172A] mb-4 text-sm uppercase mapping-wider">Company</h3>
                <ul className="space-y-3">
                  {[
              ...
              {/* Contact Column */}
              <div>
                <h3 className="font-bold text-[#0F172A] mb-4 text-sm uppercase mapping-wider">Contact</h3>
                <ul className="space-y-4">
                  <li className="flex items-start gap-2">
                    <span className="text-[#FFA600] mt-0.5">📍</span>
                    <span className="text-sm text-[#475569] leading-snug">Naroda, Gujarat, India</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-[#FFA600]">📞</span>
                    <a href="tel:+919876543210" className="text-sm text-[#475569] hover:text-[#285E89] transition-colors font-medium">+91 98765 43210</a>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-[#FFA600]">✉️</span>
                    <a href="mailto:support@h-techsolutions.in" className="text-sm text-[#475569] hover:text-[#285E89] transition-colors font-medium">support@h-techsolutions.in</a>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-[#FFA600]">🌐</span>
                    <a href="https://h-techsolutions.in" target="_blank" rel="noopener noreferrer" className="text-sm text-[#475569] hover:text-[#285E89] transition-colors font-medium">h-techsolutions.in</a>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
              <p className="text-[#475569]">© 2026 VidyaSanchalan. All rights reserved. Built by H-Tech Solutions.</p>
            </div>
          </div>
        </footer>
...
const fallbackModules = [
  ...
  { label: "GPS Attendance mapping", iconName: "Shield" },
...
const panelsData = [
  ...
  },
  {
    ...
    mockup: {
      ...
      previewText: "GR register generation tool, document uploader workspace, division allocator, and geo-mapping logs."
    }
  },
  {
    id: "fee-management",
  ...
  },
  {
    ...
    mockup: {
      ...
      previewText: "Performance progress graphs, report card download console, fee dues mapping, and school announcement logs."
    }
  }
];
